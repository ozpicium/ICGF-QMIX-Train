## last update 15 Sept 2021 by Budhitama Subagdja -- fixing the bug in perfect mismatch
## udpated 8 Sept 2021 by Budhitama Subagdja -- adding indicator for perfect mismatch
## updated 26 Aug 2021 by Budhitama Subagdja -- adding uncommitted check during resSearch to deal with template matching that can't overwrite uncommitted code
...
class FusionART:
	def __init__(self,numspace=0,lengths=[],beta=[],alpha=[],gamma=[],rho=[],schema={},numarray=True):
		...
		self.rho = list(rho)
		self.pmcriteria = [1.0]*numspace  #deprecated
		self.pmrhocriteria = [1.0]*numspace #new parameters to handle perfect mismatch 12 Sep 2021
		self.useperfectmismatch = False #new to set that perfect mismatch is applied  (disabled or false by default) 15 Sep 2021 
		...
		self.perfectMismatch = prhomismatch #new added function for checking perfect mismatch during resonance search 12092021
		...
		#self.setupInitComplFuction() #new added function for overriding the initialization value of a complement 29082021
		#------------------------------------------------------------------------ 		
	...
	def F1FieldsSetup(self, fschemas):
		...
		self.pmrhocriteria = [1.0]*len(fschemas) #new parameters to handle perfect mismatch 12 Sep 2021
	...
	#updF2 argument to enable changing the activation value of F2 in the network;  
	def resSearch(self,mtrack=[],rhos=[],F2filter=[], duprep=False, prevSel=[], extF1=None, updRec=True, updF2=True): 
		activityF1 = self.activityF1
		if extF1:
			activityF1 = copy.deepcopy(extF1)
		resetcode = True
			...
			if J >= 0:
				...
				if(not mresonance(matches,crhos)) or (J in F2filter) :
					if (not self.uncommitted(J)) or (not self.unCommitCheck): #added to handle match values that can't return 1 (e.g ART2)
						...
						#print('self.pmrhocriteria ', self.pmrhocriteria)
						if self.useperfectmismatch and self.perfectMismatch(crhos,self.pmrhocriteria):
							if updRec:
								self.perfmismatch = True #added to indicate perferct mismatch status
							return J 
				if updRec:
					self.lastActRho = list(crhos)
	...
	def SchemaBasedSequentialResSearch(self,mtrack=[],rhos=[],F2filter=[], tau=0.1, tresh=0.0, maxv=0.9, stau=0.0, accdig=10, duprep=True):
		J = self.resSearch(mtrack, rhos, F2filter)
		if hasattr(self, 'TopFusionART'):
			fTop = self.TopFusionART			
