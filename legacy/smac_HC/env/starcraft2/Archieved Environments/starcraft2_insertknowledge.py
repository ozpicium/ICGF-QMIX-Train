...
import time
# TODO these are the 4 races aviliable
...
class StarCraft2Env_insertkknowledge(MultiAgentEnv):
    ...
    def _launch(self):
        ...
        self._controller.join_game(join) # join the game. now 2 n_ally_platoons are in the map, but frozen.
    ...
    def reward_battle(self):
        ...
        for al_id, al_unit in self.agents.items():
            ...
            last_distance = math.sqrt((self.previous_ally_units[al_id].pos.x - self.reward_StrategicPoint_loc[0])**2 + (self.previous_ally_units[al_id].pos.y - self.reward_StrategicPoint_loc[1])**2)
            current_distance = math.sqrt((al_unit.pos.x - self.reward_StrategicPoint_loc[0])**2 + (al_unit.pos.y - self.reward_StrategicPoint_loc[1])**2)
            reward_ReachStrategicPoints += (last_distance-current_distance)/initial_distance * self.reward_StrategicPoint_val
        #    for i in range(4):
        #        if abs(al_unit.pos.x - self.areaCenter[i][0]) < r and abs(al_unit.pos.y - self.areaCenter[i][1]) < r:
        #            if self.visitRecord[al_id][i] == 0:
        #                self.visitRecord[al_id][i] = 1
        ...
        if self.reward_only_positive:
            ...
            reward = self.alpha * abs(delta_enemy + delta_deaths) + (1-self.alpha) * reward_ReachStrategicPoints  # shield regeneration
            ...
            reward = self.alpha * (delta_enemy + delta_deaths - delta_ally) + (1-self.alpha) * reward_ReachStrategicPoints
        return reward
    ...
    def unit_shoot_range(self, agent_id):
        ...
        if self.map_name == 'Sandbox-8t' or '8t' or '8t-s11a7':
            return 7
        else:
            return 6
    ...
    def get_surrounding_height(self, unit):
        ...
        points = self.get_surrounding_points(unit, include_self=True) # if include_self is true, return 9 points, diamond shape
        vals = [
    ...
    # 2021-08-05: get the direction of command
    def get_direction_command(self, unit):
        vals = [0,0,0,0] # go east, go south, go west, go north
        if unit.pos.x < self.reward_StrategicPoint_loc[0]:
            vals[0] = 1 # should go east
        if unit.pos.x > self.reward_StrategicPoint_loc[0]:
            vals[2] = 1  # should go west
        if unit.pos.y < self.reward_StrategicPoint_loc[1]:
    ...
    def init_units(self):
        ...
        while True:
            ...
            if counter == self.n_enemies:
                ...
                self.max_reward = self.alpha * self.max_reward + (1 - self.alpha) * self.n_agents * self.reward_StrategicPoint_val
