...
#prhomismatch: check if given matching criteria mcriteria, it's in perfect mismatch based on rho. newly added 12Sep2021
#return True or False
def prhomismatch(rhos,mcriteria):
	if type(rhos).__module__ == 'numpy':
		return np.all(rhos >= mcriteria)
	else:
		return all([rhos[k] >= mcriteria[k] for k in range(len(rhos))])
